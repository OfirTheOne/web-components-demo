...
export const internalRender: InternalRender = (
  vElem,
...
const virtualRender = (
  ...
  if (typeof tag === "function" && isPresentable(tag)) {
    element = handlePresentableElement(
      tag,
      props,
      children,
      parentPreservedStateMap,
    ...
    element = handleNativeTagElement(
      tag,
      props,
      children,
      parentPreservedStateMap,
...
const handleNativeTagElement = (
  ...
  if (children.length > 0) {
    children
      ...
      .forEach((child) => appendDomChildren(element, child));
  }
  return element;
};

const appendDomChildren = (parent: HTMLElement, child: DomElement) => {
  if (child) {
